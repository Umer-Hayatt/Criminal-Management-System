...
<div class="card" style="margin-bottom:14px;padding:18px 20px">
 <div class="search-bar-row">
  <div class="search-main">
   <label>Search</label>
   <input type="text" id="sq" placeholder="Search criminals, officers, cases, hearings, warrants..." oninput="doSearch()">
  </div>
 ...
 <div class="search-filters">
  ...
  <div class="form-group"><label>Status</label><input type="text" id="sstatus" placeholder="Wanted / Open" oninput="doSearch()"></div>
  <div class="form-group"><label>From</label><input type="date" id="sfrom" onchange="doSearch()"></div>
  <div class="form-group"><label>To</label><input type="date" id="sto" onchange="doSearch()"></div>
 </div>
</div>
...
.result-row:hover{border-color:var(--accent);background:var(--surface-alt);}
...
.et-warrant{background:rgba(34,197,94,.12);color:var(--success);}
</style>
...
function renderResults(r){
 ...
 el.innerHTML=r.results.map(x=>{
  ...
  var photo = x.photo && x.photo !== '' ? x.photo : 'assets/anon.png';
  var avatar = `<img src="${photo}" style="width:40px;height:40px;border-radius:50%;object-fit:cover;flex-shrink:0;border:1px solid var(--border)" onerror="this.src='assets/anon.png'">`;
